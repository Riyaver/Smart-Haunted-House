...
while True:
    ...
    try:
        # PIR SENSOR
        if not sensor_state["pir_triggered_3_people"]:
            ...
            if pir_value == 1 and previous_pir_value == 0:
                ...
                print(
                    f"People detected: "
                    f"{pir_people_count}/{NUMBER_OF_PEOPLE}"
                )
            ...
            if pir_people_count >= NUMBER_OF_PEOPLE:
                ...
                sensor_state["pir_triggered_3_people"] = True
                sensor_state["door_sensor_active"] = True

                client.publish(
                    "house/state/pir_triggered_3_people",
                    json.dumps({
                        "state_name": "pir_triggered_3_people",
                        "value": True
                    })
                )

                client.publish(
                    "house/state/door_sensor_active",
                    json.dumps({
                ...
                print("Published: pir_triggered_3_people = True")
                print("Published: door_sensor_active = True")


        #ALL SENSORS ACTIVE 
        if sensor_state["pir_triggered_3_people"]:
            ...
            if not sensor_state["angle_g1_sensor_active"]:
                ...
                if raw_angle_value >= ANGLE_THRESHOLD:
                    ...
                    client.publish(
                        "house/state/angle_g1_sensor_active",
                        json.dumps({
                            "state_name":
            ...
            if not sensor_state[
                ...
                raw_light_value = grovepi.analogRead(
                    LIGHT_SENSOR_PORT
                )
                ...
                if raw_light_value <= LIGHT_THRESHOLD:
                    ...
                    client.publish(
                        "house/state/light_g6_sensor_active",
                        json.dumps({
                            "state_name":
                                "light_g6_sensor_active",
                            "value": True
